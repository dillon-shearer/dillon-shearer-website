...
- If a file is renamed or moved, update the Output Manifest to remove stale paths and add the new path.
- If you re-run the same verification, update the existing entry instead of appending a duplicate.
...
Job:
- Implement only what is in scope + acceptance criteria.
- Update Task Sheet: Status = Implementing, list files touched, add verification commands you ran (or would run).
- Add Implementer Notes: what changed by file.

If anything blocks you, write it in Notes and propose the smallest workaround.
